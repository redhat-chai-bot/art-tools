    ...
    async def scan_builders_changes(self, image_meta: ImageMetadata):
        ...
        for builder in builders:
            ...
            if builder.image:
                ...
                if builder.builder:
                    # This is the parent 'from' config whose nested builder entries
                    # are already processed individually in this loop; skip the
                    # parent itself since it has no direct image or stream to track.
                    continue
            ...
            if not builder_build_nvr:
                if builder.stream:
                    if stream_config and stream_config.skip_nvr_check:
                        self.logger.warning(
                            'Skipping NVR check for stream builder %s (skip_nvr_check is set in streams.yml)',
                            builder_image_name,
                        )
                        continue
